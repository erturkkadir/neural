...
) ENGINE=InnoDB AUTO_INCREMENT=14781320 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
) ENGINE=InnoDB AUTO_INCREMENT=19234 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
if(x <= px/2) then		/* only forward */
	if(nr_type='input') then 
		set n_x = dx + myrandom(0, dx+1);
	else 
		set n_x = myrandom(-1, dx);
    end if;
end if;

if(x>(px/2-dx)) then		/* only backward */	
	if(nr_type='input') then 
		set n_x = -(dx + myrandom(0, dx+1));
	else 
		set n_x = myrandom(-dx, -1);
	end if;
end if;    

if(nr_type='output') then 
	set n_x = -1; 
end if; /* output do not go anywhere*/
...
while z<pz do 			
	...
	while y<py do 		
    	...
    	while x<px do 	
            ...
            while i<n_conn do 
				set to_x = get_position(n_type, px, d_x, x);
				set to_y = get_position(n_type, py, d_y, y);
				set to_z = get_position(n_type, pz, d_z, z);
...
/* There is no selft connection allows */
CREATE TEMPORARY TABLE IF NOT EXISTS tmp ENGINE=MEMORY AS 
...
where
 ...
 n2.nr_x = w.we_tox and 
 n2.nr_y = w.we_toy and 
 n2.nr_z = w.we_toz ; 
...
-- Dump completed on 2022-01-11 14:39:56
