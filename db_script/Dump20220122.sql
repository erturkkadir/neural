...
/*!50503 SET character_set_client = utf8mb4 */;
CREATE TABLE `route` (
  `x1` int NOT NULL COMMENT 'from neuron x',
  `y1` int NOT NULL COMMENT 'from neuron y',
  `z1` int NOT NULL COMMENT 'from neuron z',
  `x2` int NOT NULL COMMENT 'to neuron x',
  `y2` int NOT NULL COMMENT 'to neuron y',
  `z2` int NOT NULL COMMENT 'to neuron z',
  `nr_type` enum('input','hidden','output') NOT NULL DEFAULT 'hidden' COMMENT 'neuron type (input, hidden or output)',
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`kadir`@`%` FUNCTION `get_position`(
 `nr_type` varchar(20),
 `px` INTEGER,
 `dx` INTEGER,
 `ix` INTEGER,
 `x` INTEGER
    ...
    NO SQL
BEGIN
/* n_p is the new position on the given direction */
declare n_p int;

/* ix = 1 (Input kalinligi)   dx = 0 (level kalinligi)   (2, 3)      (-2  2)*/

if(x <= px/2) then		/* only forward */
	if(nr_type='input') then 
        /*if dx = 0 then return x; end if; */
		return myrandom(ix, ix + dx - 1); 
	end if;
    
    if(nr_type='hidden') then 
		return x + myrandom(-1, dx); /* Hidden cells can only go 1 layer back but dx layer forward  */
	end if;
    
    if(nr_type='output') then 
		return -1; 
	end if;
end if;

if(x > px/2) then		/* only backward */	
...
BEGIN
  ...
  RETURN round(pmin+(RAND()*(pmax-pmin)), 0);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*  wp = w - r * da/dw * dc/da 
   da/dw = nr_out * (1-nr_out)
   dc/da = 2 * (nr_out-nr_des)
*/

/* da/dw * dc/da */
/* OUTPUT LAYER ONLY da/dw * dc/da =  (a*(1-a) * 2 * (a-t)) */
/* in the case of complex plane 
   y = 1 / (1 + exp(-ix)) 
   y' = sin(x) / y2  +  i cos(x) / y2
   it will be implement in the version 2
   */
update neuron set 
	nr_delr = nr_outr*(1.0-nr_outr) * 2 * (nr_outr - nr_desr),
	nr_deli = nr_outi*(1.0-nr_outi) * 2 * (nr_outi - nr_desi),
    nr_shiftr = nr_shiftr - (nr_outr*(1.0-nr_outr) * 2 * (nr_outr - nr_desr) ),
    nr_shifti = nr_shifti - (nr_outi*(1.0-nr_outi) * 2 * (nr_outi - nr_desi) ),
    nr_status = 0    
where 
	nr_type='output' and nr_train = 1 and nr_status=1; /* train reached here */
    
/* ANY OTHER LAYER */    
/* n1 ---> wei ---> n2 */    
update 
	neuron n1, weight w, neuron n2  /* n2 is the last one, n1 is the previous*/
set 
	n1.nr_delr = n2.nr_delr * n1.nr_outr * (1.0 - n1.nr_outr),
    n1.nr_deli = n2.nr_deli * n1.nr_outi * (1.0 - n1.nr_outi),
    w.we_real = w.we_real - (n2.nr_delr * n1.nr_outr * (1.0 - n1.nr_outr)),
    w.we_imag = w.we_imag - (n2.nr_deli * n1.nr_outi * (1.0 - n1.nr_outi)),    
    n1.nr_train = (case n1.nr_type when 'input' then 0 else 1 end) ,
    n2.nr_train = 0
where
	n2.nr_train = 1 and 
	n2.nr_x = w.we_tox and n2.nr_y = w.we_toy and n2.nr_z = w.we_toz and 
	w.we_fromx = n1.nr_x and w.we_fromy = n1.nr_y and w.we_fromz = n1.nr_z and 
    n1.nr_type <> 'output';
...
set x_min = (px-cen_width)/2; /* 11 - 2/2*/
...
/* i_x, i_y, i_z is the width of input layer direction respectively				*/
...
while z<pz do 			
	...
	while y<py do 		
		set x = 0;
    	while x<px do 
			/* select nr_type into n_type from neuron where nr_x = x and nr_y = y and nr_z = z; */
            
            set nx_type = 'hidden';
            set ny_type = 'hidden';
            set nz_type = 'hidden';
            
            if (x<i_x or x>=px-i_x) then 
				set nx_type = 'input';
            end if;
            if (y<i_y or y>=py-i_y) then 
				set ny_type = 'input';
            end if;
            if (z<i_z or z>=pz-i_z) then 
				set nz_type = 'input';
            end if;
            ...
            while (i<n_conn) do 
				set to_x = get_position(nx_type, px, d_x, i_x, x);
				set to_y = get_position(ny_type, py, d_y, i_y, y);
				set to_z = get_position(nz_type, pz, d_z, i_z, z);
                /*call debug_msg(concat("to_x :", to_x, "to_y :", to_y, "to_z :", to_z, " x ", x, "y", y, "z", z));*/
                
                if !(to_x = x and to_y = y and to_z = z) then 
                
					if ( to_x>=0 and to_y>=0 and to_z>=0 ) then 
            ...
            /*call debug_msg(concat("x:", to_x, " y:", to_y, " z:", to_z));*/
...
/* Since frequency is almost 200 hz, this procedure ideally	*/
/* need to be called 200 times per second 					*/
/* 															*/
...
/* assumptions : max frequency is 200 Hz hence 				*/
/* dt = 1/400 = 0.0025 										*/
/* A typical neuron can recover in 5 ms, so if input signal	*/
/* comes within this relaxation period it is disregarded	*/
/* 5 ms is 2 timesteps 										*/
/* ******************************************************* 	*/
...
(SELECT 
	t.x, t.y, t.z, t.nr_type, t.nr_status,
	sigm_real(
...
FROM
	(select distinct w.we_tox as x, w.we_toy as y, w.we_toz as z, n0.nr_shiftr, n0.nr_shifti, n0.nr_type, n0.nr_status 
    from neuron n1, weight w, neuron n0 where n1.nr_status = 1 and 
		n1.nr_x = w.we_fromx and n1.nr_y = w.we_fromy and n1.nr_z = w.we_fromz and 
        w.we_tox = n0.nr_x and w.we_toy = n0.nr_y and w.we_toz = n0.nr_z and n0.nr_status = 0) as t, /* update olacaklar */
    weight w, neuron n
WHERE 
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `init_network` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`kadir`@`%` PROCEDURE `init_network`(
 IN t0 int
)
BEGIN
update global set time_stamp = t0;
update neuron set nr_fired_at=0, nr_status = 0, nr_inpr = 0, nr_outr = 0, nr_outr = 0, nr_outi = 0, nr_train = 0; /* everything  */
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
